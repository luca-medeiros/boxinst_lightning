...
from layers import sigmoid_focal_loss, reduce_mean, compute_ious, ml_nms, IOULoss
...
class FCOSOutputs(nn.Module):
    def __init__(self, cfg):
        ...
        self.center_sample = cfg.model.rpn.fcos.center_sample
        self.radius = cfg.model.rpn.fcos.pos_radius
        self.pre_nms_thresh_train = cfg.model.rpn.fcos.inference_th_train
        self.pre_nms_topk_train = cfg.model.rpn.fcos.pre_nms_topk_train
        self.post_nms_topk_train = cfg.model.rpn.fcos.post_nms_topk_train
        self.loc_loss_func = IOULoss(cfg.model.rpn.fcos.loc_loss_type)

        self.pre_nms_thresh_test = cfg.model.rpn.fcos.inference_th_test
        self.pre_nms_topk_test = cfg.model.rpn.fcos.pre_nms_topk_test
        self.post_nms_topk_test = cfg.model.rpn.fcos.post_nms_topk_test
        self.nms_thresh = cfg.model.rpn.fcos.nms_th
        self.thresh_with_ctr = cfg.model.rpn.fcos.thresh_with_ctr
        self.box_quality = cfg.model.rpn.fcos.box_quality

        self.num_classes = cfg.model.rpn.fcos.num_classes
        self.strides = cfg.model.rpn.fcos.fpn_strides

        # generate sizes of interest
        soi = []
        prev_size = -1
        for s in cfg.model.rpn.fcos.sizes_of_interest:
            soi.append([prev_size, s])
            prev_size = s
        soi.append([prev_size, INF])
        self.sizes_of_interest = soi

        self.loss_normalizer_cls = cfg.model.rpn.fcos.loss_normalizer_cls
        assert self.loss_normalizer_cls in ("moving_fg", "fg", "all"), \
            'model.rpn.fcos.loss_normalizer_cls can only be "moving_fg", "fg", or "all"'
        ...
        self.loss_weight_cls = cfg.model.rpn.fcos.loss_weight_cls

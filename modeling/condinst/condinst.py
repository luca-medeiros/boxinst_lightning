...
from layers import aligned_bilinear
from structures import ImageList, Instances
from modeling.backbone.build_backbone import build_fpn_backbone
from modeling.fcos.build_proposal_generator import build_proposal_generator

from structures import PolygonMasks, polygons_to_bitmask
...
class CondInst(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.backbone = build_fpn_backbone(cfg)
        self.proposal_generator = build_proposal_generator(cfg)
        self.mask_head = build_dynamic_mask_head(cfg)

        self.mask_branch = build_mask_branch(cfg, self.backbone.output_shape())

        self.mask_out_stride = cfg.model.head.condinst.mask_out_stride

        self.max_proposals = cfg.model.head.condinst.max_proposals
        self.topk_proposals_per_im = cfg.model.head.condinst.topk_proposals_per_im

        self.boxinst_enabled = cfg.model.head.boxinst.enabled
        self.bottom_pixels_removed = cfg.model.head.boxinst.bottom_pixels_removed
        self.pairwise_size = cfg.model.head.boxinst.pairwise.size
        self.pairwise_dilation = cfg.model.head.boxinst.pairwise.dilation
        self.pairwise_color_thresh = cfg.model.head.boxinst.pairwise.color_thresh
        ...
        pixel_mean = torch.Tensor(cfg.data.means).view(3, 1, 1)
        pixel_std = torch.Tensor(cfg.data.std).view(3, 1, 1)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
        ...
        elif gt_instances is not None:
            pred_instances_w_masks = self._forward_mask_heads_test(
                proposals['proposals'], mask_feats)
            mask_losses = self._forward_mask_heads_train(
                proposals, mask_feats, gt_instances)
            losses = {}
            losses.update(sem_losses)
            ...
            pred_instances_w_masks = self._forward_mask_heads_test(
                proposals['proposals'], mask_feats, None)
        ...
        for per_im_gt_inst in instances:
            ...
            if isinstance(per_im_gt_inst.get("gt_masks"), PolygonMasks):
                ...
                for per_polygons in polygons:
                    ...
                    bitmask = torch.from_numpy(bitmask).float()
                    start = int(self.mask_out_stride // 2)
    ...
    def add_bitmasks_from_boxes(self, instances, images, image_masks, im_h, im_w):
        ...
        for im_i, per_im_gt_inst in enumerate(instances):
            ...
            per_im_gt_inst.gt_bitmasks = torch.stack(per_im_bitmasks, dim=0).to(per_im_boxes.device)
            per_im_gt_inst.gt_bitmasks_full = torch.stack(
                per_im_bitmasks_full, dim=0).to(per_im_boxes.device)
    ...
    def visualize_feats(self, mask_feats, bb):
        import numpy as np
        import matplotlib.pyplot as plt
        import cv2
        for k in range(16):
            feature_image = mask_feats[0, k, :, :].detach().cpu().numpy()
            ...
            x1, y1, x2, y2 = bb[k][1:]
            cv2.rectangle(feature_image, (x1, y1), (x2, y2), (255, 0, 0), 1)

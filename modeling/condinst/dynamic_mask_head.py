...
from layers import compute_locations, aligned_bilinear
...
def compute_pairwise_term(mask_logits, pairwise_size, pairwise_dilation):
    ...
    from modeling.condinst.condinst import unfold_wo_center
...
class DynamicMaskHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_layers = cfg.model.head.condinst.mask_head.num_layers
        self.channels = cfg.model.head.condinst.mask_head.channels
        self.in_channels = cfg.model.head.condinst.mask_branch.out_channels
        self.mask_out_stride = cfg.model.head.condinst.mask_out_stride
        self.disable_rel_coords = cfg.model.head.condinst.mask_head.disable_rel_coords

        soi = cfg.model.rpn.fcos.sizes_of_interest
        ...
        self._warmup_iters = cfg.model.head.boxinst.pairwise.warmup_iters
    ...
    def mask_heads_forward_with_coords(
        ...
        mask_logits = self.mask_heads_forward(
            mask_head_inputs, weights, biases, n_inst)

        mask_logits = mask_logits.reshape(-1, 1, H, W)

        assert mask_feat_stride >= self.mask_out_stride
        assert mask_feat_stride % self.mask_out_stride == 0
        mask_logits = aligned_bilinear(mask_logits, int(
            mask_feat_stride / self.mask_out_stride))
    ...
    def __call__(self, mask_feats, mask_feat_stride, pred_instances, gt_instances=None):
        if gt_instances is not None:
            ...
            gt_bitmasks = gt_bitmasks[gt_inds].unsqueeze(
                dim=1).to(dtype=mask_feats.dtype)
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    loss_pairwise = (
                        pairwise_losses * weights).sum() / weights.sum().clamp(min=1.0)
